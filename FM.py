...
Embedding_rate = 10
SAMPLE_LENGTH = 300   # The sample length (ms)
...
File_Embed="/data1/linzn/data/ch_g729a_%d_%dms_FEAT" % (Embedding_rate,SAMPLE_LENGTH)
File_NoEmbed="/data1/linzn/data/ch_g729a_0_%dms_FEAT" % SAMPLE_LENGTH
...
def teacher_model():
    model_input = Input(shape=(int(SAMPLE_LENGTH/10),3))
...
if __name__ == '__main__':
    ...
    print("Embedding_rate,SAMPLE_LENGTH,flag_str:",Embedding_rate,SAMPLE_LENGTH,flag_str)
    pklfilex='../data_20/%s_np_all_samples_x_%d_%d.pkl'%(flag_str,Embedding_rate,SAMPLE_LENGTH)
    pklfiley='../data_20/%s_np_all_samples_y_%d_%d.pkl'%(flag_str,Embedding_rate,SAMPLE_LENGTH)
    ...
    if len(np_all_samples_x.shape) < 2 :
        index = [ i  for i in range(np_all_samples_x.shape[0]) if len(np_all_samples_x[i]) < 1]
    ...
    for i in range(iter3):
        ...
        s_model_new.save_weights('final_%s_%d_%d.h5'%(flag_str,Embedding_rate,SAMPLE_LENGTH))
